...
- Setup a deposit wallet and configure it (via web control panel)
	- Refer to Rollin.Games API Wallet User Manual for detailed steps.
- Request an API code/secret (via web control panel)
- Create deposit addresses (via Rollin.Games Wallet API)
	- Refer to [Create deposit addresses](#create-deposit-addresses) API
- Waiting for the Rollin.Games API Wallet system detecting transactions to those deposit addresses
- Handle the deposit callback
	- Use the callback data to update certain data on your system.
	- <b>Security Enhancement</b>: Use the Query Callback Detail API to confirm the callback is sent from the Rollin.Games API Wallet system.
...
- Setup a withdrawal wallet and configure it (via web control panel)
	- Refer to Rollin.Games API Wallet User Manual for detailed steps.
- Request an API code/secret (via web control panel)
- Make withdraw request (via Rollin.Games Wallet API)
	- Refer to [Withdraw Assets](#withdraw-assets) API
	- <b>Security Enhancement</b>: Also set the withdrawal authentication callback URL to authorize the withdrawal requests sent to the Rollin.Games API Wallet system.
- Waiting for the Rollin.Games API Wallet system broadcasting transactions to blockchain
- Handle the withdrawal callback
	- Use the callback data to update certain data on your system.
	- Refer to [Callback Integration](#callback-integration) for detailed information.

### Try it now
- Use [mock server](#mock-server) to test Rollin.Games Wallet API right away.
...
- Please refer to the code snippet on the github project to know how to calculate the checksum.
	...
	- [Java](https://github.com/rollingames/api-checksum-calc/blob/main/java/checksum.java#L49)
	- [PHP](https://github.com/rollingames/api-checksum-calc/blob/main/php/checksum.php#L27)
	- [C#](https://github.com/rollingames/api-checksum-calc/blob/main/c%23/checksum.cs#L55)
	- [Python](https://github.com/rollingames/api-checksum-calc/blob/main/python/checksum.py#L29)
...
- To ensure that the callbacks have processed by callback handler, the Rollin.Games API Wallet system will continue to send the callbacks to the callback URL until a callback confirmation (HTTP/1.1 200 OK) is received or exceeds the number of retries (retry time interval: 1-3-5-15-45 mins).
	- If all attempts fail, the callback will be set to a failed state, the callback handler can call the [resend deposit callback](#resend-deposit-callbacks) or [resend withdrawal callback](#resend-withdrawal-callbacks) API to request Rollin.Games API Wallet system to resend such kind of callback(s) or through the web control panel.
...
- Please refer to the code snippet on the github project to know how to validate the callback payload.
	- [Go](https://github.com/rollingames/wallet-api-mock-server/blob/main/controllers/OuterController.go#L197)
	- [Javascript](https://github.com/rollingames/wallet-api-mock-server-javascript/blob/main/routes/wallets.js#L352)
	- [Java](https://github.com/rollingames/wallet-api-mock-server-java/blob/master/src/main/java/com/rollingames/sofa/mock/MockController.java#L93)
	- [PHP](https://github.com/rollingames/wallet-api-mock-server-php/blob/master/index.php#L207)
	- [C#](https://github.com/rollingames/api-checksum-calc/blob/main/c%23/checksum.cs#L89)
	- [Python](https://github.com/rollingames/api-checksum-calc/blob/main/python/checksum.py#L64)
...
Create deposit addresses on certain wallet. Once addresses are created, the Rollin.Games API Wallet system will callback when transactions are detected on these addresses.
...
When an abnormal deposit is detected, the Rollin.Games API Wallet system will set the deposit address to invalid. Use this API to obtain the all invalid deposit addresses for further usage.
...
To withdraw assets from an withdrawal wallet, the caller must to provide an unique **order_id** for each request, the Rollin.Games API Wallet system will send the callback with the unique **order_id** when the withdrawal is success (from `in pool` state to `in chain` state). 
...
| message | string | optional | Message (This message only saved on Rollin.Games, not sent to blockchain) |
...
| blocklist_tags | array | The tags of Rollin.Games AML detection. If `out` is true, the `to_address` is tagged. Otherwise, the `from_address` is tagged |
...
### How to compile
- Put sample code to {YOUR\_GO\_PATH}/github.com/rollingames/wallet\-api\-mock\-server
- Execute
	- glide install
...
>	Configure Rollin.Games API server URL in mockserver.app.conf
...
> 
> Refer to [WithdrawalCallback()](https://github.com/rollingames/wallet-api-mock-server/blob/main/controllers/OuterController.go#L183) function in mock server OuterController.go
...
- [Java](https://github.com/rollingames/wallet-api-mock-server-java)
...
<table>
    ...
    The extra information of this callback
	 	<table>
	 	  ...
	 	  <tbody>
		    ...
		    <tr><td>blocklist_tags</td><td>The tags of Rollin.Games AML detection</td></tr>
		    ...
		    <tr><td>aml_tags</td><td>Detailed Rollin.Games AML detection information includes score, tags and blocked flag</td></tr>
		    <tr><td>aml_screen_pass</td><td>Pass or fail Rollin.Games AML screening</td></tr>
	 	  </tbody>
		</table>
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
		...
		*pseudo cryptocurrency definition in the Rollin.Games API Wallet system
    </td>
  </tr>
  <tr>
    <td>token_address</td>
...
| :-:  | :---  | :--- | :-: | :-- |
| 0    | Init        | The withdrawal request has been enqueued in the Rollin.Games API Wallet system | - | - |
| 1    | Processing  | The withdrawal request is processing in the Rollin.Games API Wallet system | O | Withdrawal(2) |
...
> Refer to [here](https://github.com/satoshilabs/slips/blob/master/slip-0044.md) for more detailed currency definitions
> 
> The * mark represents the definition of pseudo-cryptocurrency in the Rollin.Games API Wallet system
