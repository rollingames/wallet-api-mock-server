...
Rollin.Games Merchant Service(CMS) is a comprehensive cryptocurrency payment servcie, based on the deposit wallet of Rollin.Games VAULT.

### Try it now
- Use [mock server](#mock-server) to test Rollin.Games Merchant API right away.
...
- The Rollin.Games Merchant Service verifies all incoming requests. All requests must include X-API-CODE, X-CHECKSUM headers otherwise caller will get a 403 Forbidden error.
...
- To ensure that the callbacks have processed by callback handler, the Rollin.Games Merchant Service will continue to send the callbacks to the callback URL until a callback confirmation (HTTP/1.1 200 OK) is received or exceeds the number of retries (retry time interval: 1-3-5-15-45 mins).
	- If all attempts fail, the callback will be set to a failed state, the callback handler can call the [resend failed callbacks](#resend-failed-merchant-callbacks) API to request Rollin.Games Merchant Serviceto resend such kind of callback(s) or through the web control panel.
...
> The `redirect_url` must be encoded. Please refer to the code snippet on the github project to know how to encode the URL. [Go](https://github.com/rollingames/wallet-api-mock-server/blob/main/controllers/MerchantController.go#L92), [Javascript](https://github.com/rollingames/wallet-api-mock-server-javascript/blob/main/routes/merchant.js#L45), [PHP](https://github.com/rollingames/wallet-api-mock-server-php/blob/master/index.php#L370)
...
> The `redirect_url` is encoded. Please refer to the code snippet on the github project to know how to decode the URL. [Go](https://github.com/rollingames/wallet-api-mock-server/blob/main/controllers/MerchantController.go#L129), [Javascript](https://github.com/rollingames/wallet-api-mock-server-javascript/blob/main/routes/merchant.js#L66), [JAVA](https://github.com/rollingames/wallet-api-mock-server-java/blob/master/src/main/java/com/rollingames/sofa/mock/MerchantController.java#L97), [PHP](https://github.com/rollingames/wallet-api-mock-server-php/blob/master/index.php#L381)
...
> 
> BSC is a pseudo cryptocurrency definition in the Rollin.Games API Wallet system
